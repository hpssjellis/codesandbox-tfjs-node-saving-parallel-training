...
app.post("/myaction", async (req, res) => {
  ...
  const myFileName =
    layerIndex >= 0 ? `multi-layer-${layerIndex}.txt` : "multi-layer-0.txt";

  if (req.body.myPass !== "strongpass") {
    // Placeholder check, replace with secure auth for production
    console.log("bad password");
    return res.send(
      "<body onload=\"alert('Incorrect Password'); location.replace(document.referrer);\"></body>",
    );
  }
  ...
  try {
    ...
    res.send(
      "<body onload=\"alert('File saved successfully'); location.replace(document.referrer);\"></body>",
    );
  } catch (err) {
    ...
    res
      .status(500)
      .send(
        "<body onload=\"alert('Failed to save the file.'); location.replace(document.referrer);\"></body>",
      );
  }
});
